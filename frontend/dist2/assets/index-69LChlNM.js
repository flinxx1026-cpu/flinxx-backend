import{r as A,a as Td,R as si,u as at,b as Ad,c as yc,B as Cd,d as Sd,e as ft}from"./vendor-CWYb8WqM.js";(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))r(s);new MutationObserver(s=>{for(const o of s)if(o.type==="childList")for(const a of o.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&r(a)}).observe(document,{childList:!0,subtree:!0});function t(s){const o={};return s.integrity&&(o.integrity=s.integrity),s.referrerPolicy&&(o.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?o.credentials="include":s.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function r(s){if(s.ep)return;s.ep=!0;const o=t(s);fetch(s.href,o)}})();var vc={exports:{}},ii={};/**
 * @license React
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const sy=new Ye(X.comparator);function Js(){return sy}const Lu=new Ye(X.comparator);function Is(...n){let e=Lu;for(const t of n)e=e.insert(t.key,t);return e}function Mu(n){let e=Lu;return n.forEach((t,r)=>e=e.insert(t,r.overlayedDocument)),e}function yn(){return Rr()}function Vu(){return Rr()}function Rr(){return new er(n=>n.toString(),(n,e)=>n.isEqual(e))}const iy=new Ye(X.comparator),oy=new We(X.comparator);function He(...n){let e=oy;for(const t of n)e=e.add(t);return e}const ay=new We(pe);function ly(){return ay}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class Qo{constructor(){this.Tr=new We(ke.Er),this.dr=new We(ke.Ar)}isEmpty(){return this.Tr.isEmpty()}addReference(e,t){const r=new ke(e,t);this.Tr=this.Tr.add(r),this.dr=this.dr.add(r)}Rr(e,t){e.forEach(r=>this.addReference(r,t))}removeReference(e,t){this.Vr(new ke(e,t))}mr(e,t){e.forEach(r=>this.removeReference(r,t))}gr(e){const t=new X(new Ae([])),r=new ke(t,e),s=new ke(t,e+1),o=[];return this.dr.forEachInRange([r,s],a=>{this.Vr(a),o.push(a.key)}),o}pr(){this.Tr.forEach(e=>this.Vr(e))}Vr(e){this.Tr=this.Tr.delete(e),this.dr=this.dr.delete(e)}yr(e){const t=new X(new Ae([])),r=new ke(t,e),s=new ke(t,e+1);let o=He();return this.dr.forEachInRange([r,s],a=>{o=o.add(a.key)}),o}containsKey(e){const t=new ke(e,0),r=this.Tr.firstAfterOrEqual(t);return r!==null&&e.isEqual(r.key)}}class ke{constructor(e,t){this.key=e,this.wr=t}static Er(e,t){return X.comparator(e.key,t.key)||pe(e.wr,t.wr)}static Ar(e,t){return pe(e.wr,t.wr)||X.comparator(e.key,t.key)}}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
`),!1}},[S]);A.useEffect(()=>{or.current={reinitializeCamera:es}},[es]),A.useEffect(()=>{if(!le)return;const P=setInterval(()=>{_(M=>M+1)},1e3);return()=>clearInterval(P)},[le]),A.useEffect(()=>{U&&S&&console.log("🎬 [PARTNER FOUND] Transitioning to video chat screen")},[U,S]);const lr=async()=>{var P;try{if(console.log("📹 [START CAMERA] User clicked to start camera"),console.log("📹 [START CAMERA] Checking DOM state..."),console.log("   localVideoRef.current:",!!H.current),console.log("   localVideoRef.current in DOM:",(P=H.current)!=null&&P.parentElement?"YES":"NO"),console.log("   All videos in DOM:",document.querySelectorAll("video").length),!H.current){console.error("📹 [START CAMERA] ❌ Video element not in DOM - cannot proceed");return}console.log("📹 [START CAMERA] Requesting camera from browser...");const M=await navigator.mediaDevices.getUserMedia({video:!0,audio:!1});Wt.current=M,te.current=M,console.log("📹 [START CAMERA] ✅ Stream obtained:",M),console.log("📹 [START CAMERA] Stream tracks:",M.getTracks().map(F=>({kind:F.kind,enabled:F.enabled,id:F.id}))),H.current&&(H.current.srcObject=M,console.log("📹 [START CAMERA] ✅ Stream attached to video element"),H.current.onloadedmetadata=()=>{console.log("📹 [START CAMERA] ✅ Video metadata loaded, calling play()"),H.current.play().then(()=>{console.log("📹 [START CAMERA] ✅ Video playing - setting isLocalCameraReady=true"),Rt(!0)}).catch(F=>{console.warn("📹 [START CAMERA] ⚠️ Play warning (but video loaded):",F.message),Rt(!0)})})}catch(M){console.error("📹 [START CAMERA] ❌ CRITICAL ERROR:",M),console.error("   Error name:",M.name),console.error("   Error message:",M.message),M.name==="NotAllowedError"?console.error("❌ Camera permission DENIED by user - User clicked deny in browser prompt"):M.name==="NotFoundError"?console.error("❌ No camera device found - Check if device has a camera"):M.name==="NotReadableError"?console.error("❌ Camera is already in use by another app - Close other apps using camera"):M.name==="SecurityError"&&console.error("❌ Camera access blocked by security policy - Must use HTTPS")}};A.useEffect(()=>()=>{console.log("📹 [FINAL CLEANUP] Component unmounting - stopping all streams"),stopLocalCamera(),Wt.current&&(Wt.current.getTracks().forEach(P=>{console.log("📹 [FINAL CLEANUP] Stopping track:",P.kind),P.stop()}),Wt.current=null)},[]),A.useEffect(()=>{console.log("📹 [AUTO-START] Dashboard mounted - starting camera for preview"),(async()=>{try{if(console.log("📹 [AUTO-START] Requesting camera for dashboard preview..."),!H.current){console.error("📹 [AUTO-START] ❌ Video element not in DOM, cannot initialize camera");return}console.log("📹 [AUTO-START] ✓ Video element found in DOM, requesting camera permissions");const M=await navigator.mediaDevices.getUserMedia({video:{width:{ideal:640},height:{ideal:480}},audio:!0});te.current=M,Wt.current=M,console.log("📹 [AUTO-START] ✅ Camera stream obtained:",M),console.log("📹 [AUTO-START] Stream tracks:",M.getTracks().map(F=>({kind:F.kind,id:F.id}))),H.current&&(H.current.srcObject=M,H.current.muted=!0,H.current.onloadedmetadata=()=>{console.log("📹 [AUTO-START] ✅ Video metadata loaded, calling play()"),H.current.play().then(()=>{console.log("📹 [AUTO-START] ✅ Video playing - camera ready on dashboard"),N(!0),Rt(!0)}).catch(F=>{console.warn("📹 [AUTO-START] ⚠️ Play warning (but video loaded):",F.message),N(!0),Rt(!0)})})}catch(M){console.error("📹 [AUTO-START] ❌ Camera error:",M.message),console.error("📹 [AUTO-START] Error name:",M.name),Rt(!0)}})()},[]);const cr=async()=>{if(console.log("🔧 createPeerConnection called"),console.log("   Current localStreamRef:",te.current),!te.current){console.warn("⚠️ CRITICAL: localStreamRef.current is null - attempting to reacquire camera stream");try{const ee=await navigator.mediaDevices.getUserMedia({video:{width:{ideal:640},height:{ideal:480}},audio:!0});if(te.current=ee,H.current){H.current.srcObject=ee,H.current.muted=!0;try{await H.current.play()}catch($){console.warn("⚠️ Play error during reacquisition:",$)}}console.log("✅ LOCAL STREAM RE-ACQUIRED SUCCESSFULLY"),console.log("   Tracks:",ee.getTracks().map($=>({kind:$.kind,id:$.id})))}catch(ee){throw console.error("❌ FATAL: Could not reacquire camera stream:",ee.message),new Error("Cannot proceed: local camera stream unavailable - "+ee.message)}}AI();const P=await ts(),M=[{urls:["stun:global.xirsys.net","turn:global.xirsys.net:3478?transport=udp","turn:global.xirsys.net:3478?transport=tcp"],username:"nkhlvdv",credential:"a8e244b8-cf5b-11f0-8771-0242ac140002"},...P];console.log("🔧 ICE Servers Configuration:",{count:M.length,servers:M.map(ee=>({urls:ee.urls,username:ee.username?"***":void 0,credential:ee.credential?"***":void 0}))});const F=new RTCPeerConnection({iceServers:M,iceTransportPolicy:"all"});if(z.current=F,console.log("✅ RTCPeerConnection created with iceTransportPolicy: all"),F.onicecandidate=ee=>{if(ee.candidate){const $=ee.candidate;console.log("🧊 ICE candidate generated:",{candidate:$.candidate,protocol:$.protocol,port:$.port,address:$.address,type:$.type,priority:$.priority,sdpMLineIndex:$.sdpMLineIndex,sdpMId:$.sdpMid}),$.type==="relay"?(console.log("🔄 RELAY (TURN) candidate generated - TURN server is reachable"),console.log("   Protocol:",$.protocol,"Port:",$.port)):$.type==="srflx"?(console.log("📍 SRFLX (server reflexive) candidate - STUN working"),console.log("   Found public address via STUN")):$.type==="host"&&console.log("🏠 HOST candidate - direct LAN connection possible"),console.log("🔌 Sending ICE candidate to partner socket:",rt.current),K.emit("ice_candidate",{candidate:$,to:rt.current}),console.log("📤 ICE candidate sent to peer")}else console.log("🧊 ICE gathering complete (null candidate received)"),console.log("📊 ICE gathering summary:"),console.log("   Connection State:",F.connectionState),console.log("   ICE Connection State:",F.iceConnectionState),console.log("   ICE Gathering State:",F.iceGatheringState)},F.oniceconnectionstatechange=()=>{const ee=F.iceConnectionState;switch(console.log(`
🧊 ===== ICE CONNECTION STATE CHANGED =====`),console.log("🧊 New ICE Connection State:",ee),ee){case"new":console.log("🧊 State: NEW - Gathering ICE candidates");break;case"checking":console.log("🧊 State: CHECKING - Testing ICE candidate pairs"),console.log("🧊 Connection in progress - waiting for connectivity");break;case"connected":console.log("✅ State: CONNECTED - Found working ICE candidate pair"),console.log("🧊 Peer-to-peer communication established");break;case"completed":console.log("✅ State: COMPLETED - ICE checks completed, ready for media"),console.log("🧊 All connectivity checks passed");break;case"failed":console.error("❌ State: FAILED - All ICE candidate pairs failed"),console.error("❌ Could not establish peer-to-peer connection"),console.error("❌ TURN server may be unreachable or blocked by ISP"),console.error("🔍 Troubleshooting:"),console.error("   - Check console for TURN error details"),console.error("   - TURN error 701 = Network/ISP blocking ports 3478, 5349"),console.error("   - Solutions: Try VPN, different WiFi, or mobile hotspot"),console.error("   - User can retry with a retry button (do NOT auto-restart ICE)");break;case"disconnected":console.warn("⚠️ State: DISCONNECTED - Lost connection to peer"),console.warn("   Note: ICE restart is manual only to prevent stream loss");break;case"closed":console.log("🛑 State: CLOSED - Connection closed");break}console.log("📊 Full connection states:"),console.log("   Signaling State:",F.signalingState),console.log("   Connection State:",F.connectionState),console.log("   ICE Gathering State:",F.iceGatheringState)},z.current._remoteStream||(z.current._remoteStream=new MediaStream,console.log("✅ PERSISTENT REMOTE STREAM CREATED - will accumulate all incoming tracks")),F.ontrack=ee=>{console.log(`

🔴🔴🔴 ===== CRITICAL: ONTRACK HANDLER FIRING! =====`),console.log("🔴 ONTRACK CALLED AT:",new Date().toISOString()),console.log("🔴 Track received:",{kind:ee.track.kind,id:ee.track.id,enabled:ee.track.enabled});const $=z.current._remoteStream;if(console.log("🔴 Using persistent remote stream ID:",$.id),$.addTrack(ee.track),console.log("✅ Track added to persistent remote stream"),console.log("📥 Remote stream now has",$.getTracks().length,"track(s)"),console.log("📥 Tracks:",$.getTracks().map(he=>({kind:he.kind,id:he.id,enabled:he.enabled}))),!Ke.current){console.error("❌ CRITICAL ERROR: remoteVideoRef.current is NULL!"),console.error("   Cannot attach remote track - video element not available");return}Ke.current.srcObject!==$?(console.log("📺 ATTACHING PERSISTENT STREAM to remoteVideoRef"),Ke.current.srcObject=$,Ke.current.muted=!1,console.log("📺 srcObject attached, attempting play()..."),Ke.current.play().catch(()=>{console.log("ℹ️ Autoplay blocked - will play on user interaction")})):(console.log("📺 STREAM ALREADY ATTACHED - skipping re-attachment"),console.log("   Stream has",$.getTracks().length,"tracks now")),console.log(`✅ ✅ ✅ ONTRACK COMPLETE - Remote stream persisted and attached

`)},F.onconnectionstatechange=()=>{console.log(`
🔌 ===== CONNECTION STATE CHANGED =====`),console.log("🔌 New Connection State:",F.connectionState),console.log("   ICE Connection State:",F.iceConnectionState),console.log("   ICE Gathering State:",F.iceGatheringState),console.log("   Signaling State:",F.signalingState),F.connectionState==="connected"?(L(!0),console.log("✅ WebRTC connection ESTABLISHED"),setTimeout(()=>{console.log(`
📊 ===== RECEIVER DEBUG CHECK (after connected) =====`);const ee=F.getReceivers();console.log("📊 Total receivers:",ee.length),ee.forEach((he,_e)=>{var ye,Fe,xe,Ee,de;console.log(`📊 Receiver ${_e}:`,{kind:(ye=he.track)==null?void 0:ye.kind,enabled:(Fe=he.track)==null?void 0:Fe.enabled,readyState:(xe=he.track)==null?void 0:xe.readyState,id:(Ee=he.track)==null?void 0:Ee.id,muted:(de=he.track)==null?void 0:de.muted})}),console.log("📊 Audio and video tracks should be present above");const $=F.getSenders();console.log(`
📊 Total senders:`,$.length),$.forEach((he,_e)=>{var ye,Fe,xe,Ee;console.log(`📊 Sender ${_e}:`,{kind:(ye=he.track)==null?void 0:ye.kind,enabled:(Fe=he.track)==null?void 0:Fe.enabled,readyState:(xe=he.track)==null?void 0:xe.readyState,id:(Ee=he.track)==null?void 0:Ee.id})})},1e3)):F.connectionState==="disconnected"?(L(!1),console.log("⚠️ WebRTC connection DISCONNECTED")):F.connectionState==="failed"?(L(!1),console.log("❌ WebRTC connection FAILED")):F.connectionState==="closed"&&(L(!1),console.log("❌ WebRTC connection CLOSED"))},!te.current)throw console.error("❌ CRITICAL ERROR: localStreamRef.current is null/undefined in createPeerConnection!"),new Error("Local stream lost before createPeerConnection");return F};A.useEffect(()=>(console.log(`
...
🔄 ANSWERER: Setting remote description (offer from offerer)`),await z.current.setRemoteDescription(new RTCSessionDescription(P.offer)),console.log("✅ ANSWERER: Remote description set successfully"),console.log("🎬 ANSWERER: Creating answer");const M=await z.current.createAnswer({offerToReceiveVideo:!0,offerToReceiveAudio:!0});console.log("✅ ANSWERER: Answer created with receive constraints"),console.log("📋 ANSWERER SDP CHECK - Looking for a=sendrecv:");const F=M.sdp.split(`
`).filter($=>$.includes("sendrecv")||$.includes("recvonly")||$.includes("sendonly"));console.log("   Media direction lines:"),F.forEach($=>console.log("   ",$)),console.log("🔄 ANSWERER: Setting local description (answer)"),await z.current.setLocalDescription(M),console.log("✅ ANSWERER: Local description set successfully"),console.log(`
📋 ===== ANSWERER SENDING ANSWER =====`);const ee=z.current.getSenders();console.log("📤 ANSWERER: Final senders count:",ee.length),console.log("📤 ANSWERER: Sending answer with tracks:",ee.map($=>{var he,_e,ye;return{kind:(he=$.track)==null?void 0:he.kind,id:(_e=$.track)==null?void 0:_e.id,enabled:(ye=$.track)==null?void 0:ye.enabled}})),console.log("🔌 CRITICAL: Offerer socket ID from offer:",P.from),console.log("🔌 SERVER sending ANSWER to:",P.from),K.emit("webrtc_answer",{answer:z.current.localDescription,to:P.from}),console.log("📤 ANSWERER: Answer emitted to offerer via socket:",P.from),console.log(`📋 ===== ANSWERER ANSWER SENT =====
