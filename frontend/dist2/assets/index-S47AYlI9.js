...
👤 OFFERER localStream:`,Z.current);const we=Z.current.getTracks();console.log("👤 OFFERER: All available tracks:",we),console.log("📹 OFFERER tracks detail:",we.map($e=>({kind:$e.kind,id:$e.id,enabled:$e.enabled,state:$e.readyState})));const Ct=de.getSenders();if(console.log("📤 OFFERER: Existing senders count:",Ct.length),Ct.length>0)console.warn("⚠️ OFFERER WARNING: Tracks already added! Senders:",Ct.map($e=>{var Fe,mt;return{kind:(Fe=$e.track)==null?void 0:Fe.kind,id:(mt=$e.track)==null?void 0:mt.id}})),console.warn("   Not adding tracks again to avoid duplicates");else{console.log(`
📹 OFFERER: Adding ${we.length} local tracks to peer connection`),we.forEach((Fe,mt)=>{console.log(`  [${mt}] Adding ${Fe.kind} track (id: ${Fe.id}, enabled: ${Fe.enabled})`);try{const en=de.addTrack(Fe,Z.current);console.log(`  [${mt}] ✅ addTrack succeeded, sender:`,en)}catch(en){console.error(`  [${mt}] ❌ addTrack failed:`,en)}}),console.log(`
✅ OFFERER: All tracks added to peer connection`);const $e=de.getSenders();console.log("📤 OFFERER senders count:",$e.length),console.log("📤 OFFERER senders after addTrack:",$e.map((Fe,mt)=>{var en,yn,gr;return{index:mt,kind:(en=Fe.track)==null?void 0:en.kind,id:(yn=Fe.track)==null?void 0:yn.id,trackExists:!!Fe.track,trackEnabled:(gr=Fe.track)==null?void 0:gr.enabled}})),console.log("🚀 OFFERER: Ready to send offer with",we.length,`tracks
...
📤 OFFERER: Sending offer with tracks:`,de.getSenders().map(we=>{var Ct,$e,Fe;return{kind:(Ct=we.track)==null?void 0:Ct.kind,id:($e=we.track)==null?void 0:$e.id,enabled:(Fe=we.track)==null?void 0:Fe.enabled}})),console.log("📤 OFFERER: Partner socket ID from data:",N.socketId),console.log("📤 OFFERER: partnerSocketIdRef.current value:",at.current),console.log("🔌🔌🔌 CRITICAL: About to emit webrtc_offer with to:",N.socketId),console.log("🔌🔌🔌 CRITICAL: Is to value empty/null/undefined?",!N.socketId),K.emit("webrtc_offer",{offer:z.current.localDescription,to:N.socketId}),console.log("✅ OFFERER: webrtc_offer emitted successfully"),console.log("✅ OFFERER: webrtc_offer sent to socket ID:",N.socketId),console.log("✅ OFFERER: webrtc_offer contains",z.current.getSenders().length,"senders"),console.log("✅ OFFERER: Sent to socket:",N.socketId)}catch(de){console.error("❌ OFFERER: Error in partner_found handler:",de),console.error("❌ OFFERER: Stack trace:",de.stack)}}),K.on("webrtc_offer",async N=>{console.log(`
...
//# sourceMappingURL=index-S47AYlI9.js.map
