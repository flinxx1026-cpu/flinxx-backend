import{r as T,a as Td,R as qr,u as ft,b as Ad,c as yc,B as Cd,d as Sd,e as yt}from"./vendor-CWYb8WqM.js";(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))r(s);new MutationObserver(s=>{for(const o of s)if(o.type==="childList")for(const a of o.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&r(a)}).observe(document,{childList:!0,subtree:!0});function t(s){const o={};return s.integrity&&(o.integrity=s.integrity),s.referrerPolicy&&(o.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?o.credentials="include":s.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function r(s){if(s.ep)return;s.ep=!0;const o=t(s);fetch(s.href,o)}})();var vc={exports:{}},oi={};/**
 * @license React
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class Qo{constructor(){this.Tr=new Xe(Le.Er),this.dr=new Xe(Le.Ar)}isEmpty(){return this.Tr.isEmpty()}addReference(e,t){const r=new Le(e,t);this.Tr=this.Tr.add(r),this.dr=this.dr.add(r)}Rr(e,t){e.forEach(r=>this.addReference(r,t))}removeReference(e,t){this.Vr(new Le(e,t))}mr(e,t){e.forEach(r=>this.removeReference(r,t))}gr(e){const t=new re(new Se([])),r=new Le(t,e),s=new Le(t,e+1),o=[];return this.dr.forEachInRange([r,s],a=>{this.Vr(a),o.push(a.key)}),o}pr(){this.Tr.forEach(e=>this.Vr(e))}Vr(e){this.Tr=this.Tr.delete(e),this.dr=this.dr.delete(e)}yr(e){const t=new re(new Se([])),r=new Le(t,e),s=new Le(t,e+1);let o=Ye();return this.dr.forEachInRange([r,s],a=>{o=o.add(a.key)}),o}containsKey(e){const t=new Le(e,0),r=this.Tr.firstAfterOrEqual(t);return r!==null&&e.isEqual(r.key)}}class Le{constructor(e,t){this.key=e,this.wr=t}static Er(e,t){return re.comparator(e.key,t.key)||ge(e.wr,t.wr)}static Ar(e,t){return ge(e.wr,t.wr)||re.comparator(e.key,t.key)}}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
`),!1}},[S]);T.useEffect(()=>{pn.current={reinitializeCamera:hr}},[hr]),T.useEffect(()=>{if(!de)return;const N=setInterval(()=>{_($=>$+1)},1e3);return()=>clearInterval(N)},[de]),T.useEffect(()=>{U&&S&&console.log("🎬 [PARTNER FOUND] Transitioning to video chat screen")},[U,S]);const ss=async()=>{var N;try{if(console.log("📹 [START CAMERA] User clicked to start camera"),console.log("📹 [START CAMERA] Checking DOM state..."),console.log("   localVideoRef.current:",!!ae.current),console.log("   localVideoRef.current in DOM:",(N=ae.current)!=null&&N.parentElement?"YES":"NO"),console.log("   All videos in DOM:",document.querySelectorAll("video").length),!ae.current){console.error("📹 [START CAMERA] ❌ Video element not in DOM - cannot proceed");return}console.log("📹 [START CAMERA] Requesting camera from browser...");const $=await navigator.mediaDevices.getUserMedia({video:!0,audio:!1});Zt.current=$,ee.current=$,console.log("📹 [START CAMERA] ✅ Stream obtained:",$),console.log("📹 [START CAMERA] Stream tracks:",$.getTracks().map(L=>({kind:L.kind,enabled:L.enabled,id:L.id}))),ae.current&&(ae.current.srcObject=$,console.log("📹 [START CAMERA] ✅ Stream attached to video element"),ae.current.onloadedmetadata=()=>{console.log("📹 [START CAMERA] ✅ Video metadata loaded, calling play()"),ae.current.play().then(()=>{console.log("📹 [START CAMERA] ✅ Video playing - setting isLocalCameraReady=true"),At(!0)}).catch(L=>{console.warn("📹 [START CAMERA] ⚠️ Play warning (but video loaded):",L.message),At(!0)})})}catch($){console.error("📹 [START CAMERA] ❌ CRITICAL ERROR:",$),console.error("   Error name:",$.name),console.error("   Error message:",$.message),$.name==="NotAllowedError"?console.error("❌ Camera permission DENIED by user - User clicked deny in browser prompt"):$.name==="NotFoundError"?console.error("❌ No camera device found - Check if device has a camera"):$.name==="NotReadableError"?console.error("❌ Camera is already in use by another app - Close other apps using camera"):$.name==="SecurityError"&&console.error("❌ Camera access blocked by security policy - Must use HTTPS")}};T.useEffect(()=>()=>{console.log("📹 [DASHBOARD CLEANUP] Component unmounting"),console.log("📹 [DASHBOARD CLEANUP] ⚠️ NOT stopping camera - will be reused on return")},[]),T.useEffect(()=>{console.log("📹 [CAMERA INIT] Starting camera initialization on mount");let N=!0;return(async()=>{try{if(ee.current)console.log("📹 [CAMERA INIT] Stream already exists - reusing existing stream");else{console.log("📹 [CAMERA INIT] Requesting camera permissions from browser...");const V=await navigator.mediaDevices.getUserMedia({video:{width:{ideal:1280},height:{ideal:720}},audio:!0});if(!N){console.log("📹 [CAMERA INIT] Component unmounted, cleaning up stream"),V.getTracks().forEach(le=>le.stop());return}ee.current=V,Zt.current=V,console.log("📹 [CAMERA INIT] ✅ Camera stream obtained"),console.log("📹 [CAMERA INIT] Active tracks:",V.getTracks().map(le=>({kind:le.kind,enabled:le.enabled})))}if(await new Promise(V=>setTimeout(V,0)),!N)return;let L=0;const J=()=>{Be&&ee.current?(console.log("📹 [CAMERA INIT] Attaching stream to video element..."),Be.srcObject=ee.current,Be.muted=!0,console.log("📹 [CAMERA INIT] Calling play() on video element"),Be.play().then(()=>{N&&(console.log("📹 [CAMERA INIT] ✅ Video stream is now playing"),k(!0),At(!0))}).catch(V=>{N&&(console.warn("📹 [CAMERA INIT] ⚠️ Play error (stream may still display):",V.name,V.message),k(!0),At(!0))})):L<50?(L++,setTimeout(J,50)):(console.error("📹 [CAMERA INIT] ❌ Video ref never became available"),console.error("   sharedVideoRef:",!!Be),console.error("   localStreamRef.current:",!!ee.current),At(!0))};J()}catch(L){N&&(console.error("📹 [CAMERA INIT] ❌ Error:",L.name,L.message),L.name==="NotAllowedError"?console.error("   → User denied camera permission"):L.name==="NotFoundError"?console.error("   → No camera device found"):L.name==="NotReadableError"&&console.error("   → Camera is in use by another app"),At(!0))}})(),()=>{N=!1}},[]),T.useEffect(()=>{const N=setInterval(()=>{Be&&Be.paused&&Be.srcObject&&(console.warn("📹 [RESUME CHECK] Video was paused! Resuming..."),Be.play().catch($=>console.warn("📹 [RESUME CHECK] Resume error:",$.message)))},500);return()=>clearInterval(N)},[]),T.useEffect(()=>{const $=setInterval(()=>{try{if(!Be||!ee.current)return;const L=ee.current.getTracks();L.forEach(J=>{J.readyState!=="ended"&&(J.enabled||(J.enabled=!0))}),!Be.srcObject&&L.length>0&&(Be.srcObject=ee.current,Be.muted=!0),Be.srcObject&&L.some(J=>J.readyState==="live")&&Be.paused&&Be.play().catch(()=>{})}catch{}},500);return()=>clearInterval($)},[]);const en=async()=>{if(console.log("🔧 createPeerConnection called"),console.log("   Current localStreamRef:",ee.current),!ee.current){console.warn("⚠️ CRITICAL: localStreamRef.current is null - attempting to reacquire camera stream");try{const J=await navigator.mediaDevices.getUserMedia({video:{width:{ideal:640},height:{ideal:480}},audio:!0});if(ee.current=J,ae.current){ae.current.srcObject=J,ae.current.muted=!0;try{await ae.current.play()}catch(V){console.warn("⚠️ Play error during reacquisition:",V)}}console.log("✅ LOCAL STREAM RE-ACQUIRED SUCCESSFULLY"),console.log("   Tracks:",J.getTracks().map(V=>({kind:V.kind,id:V.id})))}catch(J){throw console.error("❌ FATAL: Could not reacquire camera stream:",J.message),new Error("Cannot proceed: local camera stream unavailable - "+J.message)}}AI();const N=await dr(),$=[{urls:["stun:global.xirsys.net","turn:global.xirsys.net:3478?transport=udp","turn:global.xirsys.net:3478?transport=tcp"],username:"nkhlvdv",credential:"a8e244b8-cf5b-11f0-8771-0242ac140002"},...N];console.log("🔧 ICE Servers Configuration:",{count:$.length,servers:$.map(J=>({urls:J.urls,username:J.username?"***":void 0,credential:J.credential?"***":void 0}))});const L=new RTCPeerConnection({iceServers:$,iceTransportPolicy:"all"});if(z.current=L,console.log("✅ RTCPeerConnection created with iceTransportPolicy: all"),L.onicecandidate=J=>{if(J.candidate){const V=J.candidate;console.log("🧊 ICE candidate generated:",{candidate:V.candidate,protocol:V.protocol,port:V.port,address:V.address,type:V.type,priority:V.priority,sdpMLineIndex:V.sdpMLineIndex,sdpMId:V.sdpMid}),V.type==="relay"?(console.log("🔄 RELAY (TURN) candidate generated - TURN server is reachable"),console.log("   Protocol:",V.protocol,"Port:",V.port)):V.type==="srflx"?(console.log("📍 SRFLX (server reflexive) candidate - STUN working"),console.log("   Found public address via STUN")):V.type==="host"&&console.log("🏠 HOST candidate - direct LAN connection possible"),console.log("🔌 Sending ICE candidate to partner socket:",it.current),K.emit("ice_candidate",{candidate:V,to:it.current}),console.log("📤 ICE candidate sent to peer")}else console.log("🧊 ICE gathering complete (null candidate received)"),console.log("📊 ICE gathering summary:"),console.log("   Connection State:",L.connectionState),console.log("   ICE Connection State:",L.iceConnectionState),console.log("   ICE Gathering State:",L.iceGatheringState)},L.oniceconnectionstatechange=()=>{const J=L.iceConnectionState;switch(console.log(`
...
📊 ===== RECEIVER DEBUG CHECK (after connected) =====`);const J=L.getReceivers();console.log("📊 Total receivers:",J.length),J.forEach((le,pe)=>{var ke,qe,be,Pe,fe;console.log(`📊 Receiver ${pe}:`,{kind:(ke=le.track)==null?void 0:ke.kind,enabled:(qe=le.track)==null?void 0:qe.enabled,readyState:(be=le.track)==null?void 0:be.readyState,id:(Pe=le.track)==null?void 0:Pe.id,muted:(fe=le.track)==null?void 0:fe.muted})}),console.log("📊 Audio and video tracks should be present above");const V=L.getSenders();console.log(`
📊 Total senders:`,V.length),V.forEach((le,pe)=>{var ke,qe,be,Pe;console.log(`📊 Sender ${pe}:`,{kind:(ke=le.track)==null?void 0:ke.kind,enabled:(qe=le.track)==null?void 0:qe.enabled,readyState:(be=le.track)==null?void 0:be.readyState,id:(Pe=le.track)==null?void 0:Pe.id})})},1e3)):L.connectionState==="disconnected"?(M(!1),console.log("⚠️ WebRTC connection DISCONNECTED")):L.connectionState==="failed"?(M(!1),console.log("❌ WebRTC connection FAILED")):L.connectionState==="closed"&&(M(!1),console.log("❌ WebRTC connection CLOSED"))},!ee.current)throw console.error("❌ CRITICAL ERROR: localStreamRef.current is null/undefined in createPeerConnection!"),new Error("Local stream lost before createPeerConnection");return L};T.useEffect(()=>(console.log(`

🔌 ===== SOCKET LISTENERS SETUP (COMPONENT MOUNT) =====`),console.log("🔌 Setting up socket listeners - runs ONCE on component load"),console.log("🔌 Socket ID:",K.id),console.log("🔌 Socket connected:",K.connected),console.log("🔌 🔐 Using userIdRef.current for ALL ID comparisons:",he.current),K.off("partner_found"),K.off("webrtc_offer"),K.off("webrtc_answer"),K.off("ice_candidate"),K.off("receive_message"),K.off("partner_disconnected"),K.off("disconnect"),console.log("🔌 Removed old listeners (if any existed)"),K.on("partner_found",async N=>{var ke,qe,be,Pe;console.log(`

📋 ===== PARTNER FOUND EVENT RECEIVED =====`),console.log("👥 RAW DATA from server:",JSON.stringify(N,null,2)),console.log("👥 My socket ID:",K.id),console.log("👥 currentUser object:",JSON.stringify(te,null,2)),console.log("👥 userIdRef.current (SHOULD USE THIS):",he.current),console.log("👥 currentUser.googleId:",te==null?void 0:te.googleId),console.log("👥 currentUser.id:",te==null?void 0:te.id),console.log("👥 data.socketId:",N.socketId),console.log("👥 data.partnerId:",N.partnerId),console.log("👥 data.userName:",N.userName),E(!1),Z(!0),Mt(!1),console.log(`
👥 SELF-MATCH CHECK - START`);const $=he.current,L=N.partnerId;if(console.log("👥 COMPARISON VALUES:"),console.log("   myUserId type:",typeof $,"value:",$),console.log("   partnerUserId type:",typeof L,"value:",L),console.log("   Are they EQUAL?",$===L),console.log("   String comparison:",String($)===String(L)),$===L){console.error(`
❌❌❌ CRITICAL ERROR: SELF-MATCH DETECTED! ❌❌❌`),console.error("   My user ID:",$,"type:",typeof $),console.error("   Partner user ID:",L,"type:",typeof L),console.error("   Match IDs:",$===L),console.error("   These should be DIFFERENT!"),Mt(!0),console.error("   Emitting skip_user..."),K.emit("skip_user",{partnerSocketId:N.socketId}),console.error("   Emitting find_partner..."),K.emit("find_partner",{userId:he.current,userName:te.name||"Anonymous",userAge:te.age||18,userLocation:te.location||"Unknown"}),console.error("   Returning - match REJECTED");return}console.log("✅ SELF-MATCH CHECK PASSED - partner is different user"),console.log("   Accepting match and proceeding with WebRTC setup"),console.log(`👥 SELF-MATCH CHECK - END
`),it.current=N.socketId,console.log("🔌 CRITICAL: Stored partner socket ID:",it.current),console.log("🔌 CRITICAL: Verification - partnerSocketIdRef.current is now:",it.current),console.log("🎬 ABOUT TO CALL setHasPartner(true)"),B(!0),console.log("🎬 ✅ setHasPartner(true) CALLED - force attach effect should trigger");const J={...N,picture:N.userPicture||N.picture||null,userName:N.userName||N.name||"Anonymous",userLocation:N.userLocation||N.location||"Unknown",userAge:N.userAge||N.age||18};m(J),console.log("🎬 ✅ setPartnerInfo CALLED with data:",J);const V=K.id,le=N.socketId,pe=V<le;if(console.log("🔍 SOCKET ID COMPARISON:"),console.log("   My socket ID:",V),console.log("   Partner socket ID:",le),console.log("   Am I offerer? (myID < partnerID):",pe),console.log(`
...
📤 OFFERER: Sending offer with tracks:`,fe.getSenders().map(Ie=>{var Vt,Ve,je;return{kind:(Vt=Ie.track)==null?void 0:Vt.kind,id:(Ve=Ie.track)==null?void 0:Ve.id,enabled:(je=Ie.track)==null?void 0:je.enabled}})),console.log("📤 OFFERER: Partner socket ID from data:",N.socketId),console.log("📤 OFFERER: partnerSocketIdRef.current value:",it.current),console.log("🔌🔌🔌 CRITICAL: About to emit webrtc_offer with to:",N.socketId),console.log("🔌🔌🔌 CRITICAL: Is to value empty/null/undefined?",!N.socketId),K.emit("webrtc_offer",{offer:z.current.localDescription,to:N.socketId}),console.log("✅ OFFERER: webrtc_offer emitted successfully"),console.log("✅ OFFERER: webrtc_offer sent to socket ID:",N.socketId),console.log("✅ OFFERER: webrtc_offer contains",z.current.getSenders().length,"senders"),console.log("✅ OFFERER: Sent to socket:",N.socketId)}catch(fe){console.error("❌ OFFERER: Error in partner_found handler:",fe),console.error("❌ OFFERER: Stack trace:",fe.stack)}}),K.on("webrtc_offer",async N=>{console.log(`

🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉`),console.log("🎉🎉🎉 ⭐️ ANSWERER HANDLER FIRED ⭐️ 🎉🎉🎉"),console.log("🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉"),console.log("📋 ===== ANSWERER RECEIVED OFFER ====="),console.log("⭐️ ANSWERER: WEBRTC_OFFER EVENT FIRED - OFFER WAS RECEIVED"),console.log("📨 ANSWERER: Received WebRTC offer from offerer"),console.log("📨 ANSWERER: My socket ID:",K.id),console.log("📨 ANSWERER: Offer from:",N.from),console.log("📨 ANSWERER: Full data:",N),console.log("📨 ANSWERER: data.from (offerer socket ID):",N.from),it.current=N.from,console.log("🔌 CRITICAL: Stored offerer socket ID:",it.current);try{if(z.current)console.log("⚠️ ANSWERER: WARNING - peerConnectionRef already exists (should be null for answerer)");else{console.log("📍 ANSWERER: Creating new peer connection for the first time");let V;try{V=await en()}catch(le){console.error("❌ ANSWERER: Error creating peer connection:",le);return}z.current=V,console.log("✅ ANSWERER: Peer connection created")}if(console.log(`
...
🔄 ANSWERER: Setting remote description (offer from offerer)`),await z.current.setRemoteDescription(new RTCSessionDescription(N.offer)),console.log("✅ ANSWERER: Remote description set successfully"),console.log("🎬 ANSWERER: Creating answer");const $=await z.current.createAnswer({offerToReceiveVideo:!0,offerToReceiveAudio:!0});console.log("✅ ANSWERER: Answer created with receive constraints"),console.log("📋 ANSWERER SDP CHECK - Looking for a=sendrecv:");const L=$.sdp.split(`
`).filter(V=>V.includes("sendrecv")||V.includes("recvonly")||V.includes("sendonly"));console.log("   Media direction lines:"),L.forEach(V=>console.log("   ",V)),console.log("🔄 ANSWERER: Setting local description (answer)"),await z.current.setLocalDescription($),console.log("✅ ANSWERER: Local description set successfully"),console.log(`
...
📋 ===== OFFERER RECEIVED ANSWER =====`),console.log("📨 OFFERER: Received WebRTC answer from answerer"),console.log("📨 OFFERER: data.from (answerer socket ID):",N.from),console.log("📨 OFFERER: Answer SDP:",N.answer),it.current=N.from,console.log("🔌 CRITICAL: Stored answerer socket ID:",it.current);try{if(!z.current){console.error("❌ OFFERER: No peer connection available to handle answer");return}console.log(`
